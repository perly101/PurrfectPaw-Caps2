        ...
        <div class="flex-1 p-4 md:p-6 md:ml-64 mt-12 md:mt-0">
            <div class="bg-white shadow-lg rounded-xl px-4 sm:px-6 md:px-8 pt-4 sm:pt-6 pb-6 sm:pb-8 mb-4">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6">
                    <div>
                        <h2 class="text-xl md:text-2xl font-semibold text-gray-800">Clinic Details</h2>
                        <p class="text-gray-500 text-xs md:text-sm mt-1">Manage clinic information and associated account</p>
                    </div>
                    
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:space-x-3 w-full sm:w-auto">
                        <a href="{{ route('admin.clinics') }}" class="flex items-center justify-center px-3 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg transition text-sm">
                        ...
                        <form action="{{ route('admin.clinics.delete', $clinic->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this clinic? This will also delete the associated user account.');" class="w-full sm:w-auto">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="flex items-center justify-center px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition text-sm w-full">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg gap-3">
                    <div class="p-2 bg-blue-100 rounded-md mr-0 sm:mr-3 self-center sm:self-start">
                    ...
                    <form action="{{ route('admin.clinic.download', $clinic->id) }}" method="GET" class="flex flex-col sm:flex-row w-full gap-3 items-start sm:items-center" target="_blank">
                        <div class="w-full sm:mr-3 sm:flex-1">
                            <label class="block text-xs sm:text-sm font-medium text-blue-700 mb-1">Password of the account</label>
                            <input type="text" name="password" placeholder="Enter password of the account" 
                                class="border border-blue-300 rounded-lg px-3 py-2 w-full text-sm focus:ring focus:ring-blue-200 focus:border-blue-400" required>
                        </div>
                        <button type="submit" class="bg-blue-600 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-blue-700 transition flex items-center justify-center w-full sm:w-auto text-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @if(session('success'))
                    <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-3 sm:p-4 mb-4 sm:mb-6 text-sm sm:text-base">
                        {{ session('success') }}
                    </div>
                @endif

                @if ($errors->any())
                    <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-3 sm:p-4 mb-4 sm:mb-6 text-sm sm:text-base">
                ...
                <div class="bg-white p-4 sm:p-6 rounded-lg border border-gray-200 mb-6 sm:mb-8 shadow-sm">
                    <h3 class="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-1 sm:mr-2 text-indigo-600" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <form action="{{ route('admin.clinic.updateDetails', $clinic->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 sm:space-y-5">
                        @csrf
                        @method('PUT')

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Clinic Name</label>
                                <input type="text" name="clinic_name" value="{{ old('clinic_name', $clinic->clinic_name) }}" 
                                    class="border border-gray-300 rounded-lg px-3 py-2 w-full text-sm focus:ring focus:ring-indigo-200 focus:border-indigo-300" required>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                                ...
                                <input type="file" name="profile_picture" 
                                    class="border border-gray-300 rounded-lg px-3 py-2 w-full text-xs sm:text-sm focus:ring focus:ring-indigo-200 focus:border-indigo-300">
                            </div>
                        </div>

                        <div class="flex justify-end mt-4 sm:mt-6">
                            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 sm:px-5 py-2 rounded-lg transition flex items-center text-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="bg-white p-4 sm:p-6 rounded-lg border border-gray-200 shadow-sm">
                    <h3 class="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-1 sm:mr-2 text-purple-600" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" />
                        </svg>
                        Clinic Staff Account
                    </h3>
                    
                    <form action="{{ route('admin.clinic.updateAccount', $clinic->user->id) }}" method="POST" class="space-y-4 sm:space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="first_name" value="{{ old('first_name', $clinic->user->first_name) }}" 
                                    class="border border-gray-300 rounded-lg px-3 py-2 w-full text-sm focus:ring focus:ring-purple-200 focus:border-purple-300" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="middle_name" value="{{ old('middle_name', $clinic->user->middle_name) }}" 
                                    class="border border-gray-300 rounded-lg px-3 py-2 w-full text-sm focus:ring focus:ring-purple-200 focus:border-purple-300">
                            </div>
                        ...
                        <div class="border-t border-gray-200 mt-4 sm:mt-6 pt-4 sm:pt-6">
                            <h4 class="text-xs sm:text-sm font-semibold text-gray-700 mb-2 sm:mb-3">Change Password (optional)</h4>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                        ...
                        <div class="flex justify-end mt-4 sm:mt-6">
                            <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white px-4 sm:px-5 py-2 rounded-lg transition flex items-center text-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
